...
public class CryptoCompareRestCallerService {
    ...
    public func price(coin: Coin, targets: Array<Coin>) -> Observable<Dictionary<String, Float>> {
        let targetCoinsString: String = targets.reduce(into: "") { (result: inout String, coin: Coin) in
            result += "\(coin.name),"
        }
        let url = "\(urls.coinPrice.rawValue)?fsym=\(coin.name)&tsyms=\(targetCoinsString)"
